    ...
    while samples_number < 3e7:
        ...
        while (not done):
            ...
            if cumulative_timestep % update_timestep == 0 and cumulative_timestep > n_collect_steps:
                q_loss, pi_loss = agent.optimize(update_timestep, state_filter=state_filter)
                n_updates += 1
            # logging
            if cumulative_timestep % log_interval == 0 and cumulative_timestep > n_collect_steps:
                writer.add_scalar('Loss/Q-func', q_loss, n_updates)
                ...
                print(
                    'Episode {} \t QFuncLoss {} \t Samples {} \t Avg length: {} \t Test reward: {} \t Train reward: {} \t Number of Updates: {}'.format(
                         i_episode, q_loss, samples_number, avg_length, eval_reward, running_reward, n_updates))
                episode_steps = []
                episode_rewards = []
...
def main():
    ...
    agent = ddpg(seed, state_dim, action_dim)
